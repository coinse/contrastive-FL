...
list_inf = [list_project[i:i+10] for i in range(0, len(list_project), 10)]
#list_mutant = ['Lang_65','Lang_55','Lang_45','Lang_35','Lang_25','Lang_15','Lang_5']
list_mutant = []
partition = list_inf
for i, part in enumerate(partition):
    if i==0:
        list_mutant.append(part)
    else:
        new_part = list_mutant[-1].copy()
        new_part += part
        list_mutant.append(new_part)
#list_mutant = [list_project[i:i+10] for i in range(0, len(list_project), 10)]
...
model_epoch = 25
list_epoch = []
for i in range(model_epoch):
    if i % 5 == 0 and i!=0:
        list_epoch.append(str(i))
#list_epoch = [x for x in list_epoch if int(x)<=25 or int(x)>=225]
list_epoch = ['best']
acc_stat_avg = [0, 0, 0, 0]
acc_stat_min = [0, 0, 0, 0]
percentage = '0.1'
for i, e in enumerate(list_mutant):
    if i!=0:
        a = 'CROFL'
        ms = list_mutant[i-1]
        a+=ms[0].split('_')[1]+ms[-1].split('_')[1]
        a+='_'+percentage
        model_dict = torch.load(f'new-model/{model_version}/CROFL/model_{arc}_{a}_{mod}_best.pth', map_location="cuda:0")
        new_state_dict = {}
    ...
    for p in tqdm(list_project):
        if project_title in p and p not in deprecated_bugs:
            ...
            #if p == mutant_source:
            #    print('\n')
            #    print(acc1_avg, acc3_avg, acc5_avg, acc10_avg)
            #    print(acc1_min, acc3_min, acc5_min, acc10_min)
text_list.append('accstat for {}:'.format(f'new-model/{model_version}/model_{arc}_{a}_{mod}_{e} with avg'))
...
text_list.append('\n')
text_list.append('accstat for {}:'.format(f'new-model/{model_version}/model_{arc}_{a}_{mod}_{e} with min'))
...
print(f'number of uncovered method across {project_title} : {len(uncovered_collage)}, constructor : {len(uncovered_constructor)}')
os.makedirs(f'CROFL results/{model_version}', exist_ok=True)
with open(f'CROFL results/{model_version}/CROFL_{arc}_{a}_results_{mod}.txt', 'w') as file:
