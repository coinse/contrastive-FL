...
for mutant_source in list_mutant:
    ...
    mx = 0
    kt_n = 3
    print('creating dataset', mutant_source)
    ...
    for epoch in range(num_epoch):
        ...
        for batch_idx, (method_batch, test_batch, label) in tqdm(enumerate(dataloader)):
            ...
            if torch.isnan(l).any():
                ...
                torch.save(model.state_dict(), f'new-model/{project_title}/BM25/model_{arc}_lastnan.pth')
                exit()
            l.backward()
            optimizer.step()
        ...
        if avg_epoch_loss<best_val_loss:
            ...
            p_counter = 0
            os.makedirs(f'new-model/{project_title}/BM25', exist_ok=True)
            torch.save(model.state_dict(), f'new-model/{project_title}/BM25/model_{arc}_{a}_{m}_best.pth')
        else:
            p_counter+=1
        if p_counter >= 5:
            if epoch+1>30:
        ...
        #if (epoch+1)%5==0:
        #    os.makedirs(f'new-model/{project_title}/BM25', exist_ok=True)
        #    torch.save(model.state_dict(), f'new-model/{project_title}/BM25/model_{arc}_{a}_{m}_{epoch+1}.pth')
