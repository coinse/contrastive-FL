...
for project_name in tqdm(list_project):
    ...
    if project_title in project_name and project_name in mutant_source:
        ...
        with open('complete_mutant_data_new.pkl', 'rb') as mf:
            mutant = pickle.load(mf)
        with open('complete_test_data_new.pkl', 'rb') as tf:
            test = pickle.load(tf)
        with open('complete_method_data_new.pkl', 'rb') as mef:
            method = pickle.load(mef)
        os.chdir(cur)
        exclude_mutant = []
        with open(f'exclude_mutant_sample/{project_name}_{percentage}.log', newline='') as emf:
            for line in emf:
                exclude_mutant.append(int(line))
        r_dict = {}
        replacement_index = 0
        method_list = []
        for m in method:
        ...
        for mutant_no in mutant:
            if mutant[mutant_no]['killer'] and mutant_no not in exclude_mutant:
                if args.mod == 'all':
                    tl = []
                    tr = []
...
plt.savefig(f'CROFL results/version_batch/{project_title}/{arc}_{a}_newloss_{m}.png', format="png", dpi=300, bbox_inches="tight")
plt.close()
#os.makedirs(f'new-model/{project_title}/version_batch', exist_ok=True)
